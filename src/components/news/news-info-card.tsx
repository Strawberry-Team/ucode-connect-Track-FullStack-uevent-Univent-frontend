// components/news/NewsInfoCard.tsx
"use client";
...
export default function NewsInfoCard({ companyId }: NewsCardProps) {
    const [news, setNews] = useState<CompanyNews[]>([]);
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [isClicked, setIsClicked] = useState(false);
    const [isLoading, setIsLoading] = useState(true);

    // Загрузка новостей при монтировании компонента или изменении companyId
    useEffect(() => {
        const fetchNews = async () => {
            setIsLoading(true);
            const start = Date.now();
            const result = await getCompanyNewsById(companyId);
    ...
    const handleNewsCreated = (newNews: CompanyNews) => {
        setNews((prev) =>
            [...prev, newNews].sort(
                (a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
            )
